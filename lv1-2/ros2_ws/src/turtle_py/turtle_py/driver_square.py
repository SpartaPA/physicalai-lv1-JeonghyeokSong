...
class DriverSquare(Node):
    ...
    def timer_callback(self):
        ...
        if self.side >= self.get_parameter("side").value:
            msg = Twist()
            msg.linear.x = 0.0
            msg.angular.z = 0.0
            ...
            if self.phase == "forward":
                ...
                msg.linear.x = 1.0
                self.publisher.publish(msg)
                if d >= 10.0/self.get_parameter("side").value :
                    self.phase = "turn"
                    self.tick = 0
                ...
                self.tick += 1
                msg = Twist()
                msg.angular.z = math.pi*(2/self.get_parameter("side").value)
                self.publisher.publish(msg)
                if self.tick >= 10:
                    self.phase = "forward"
                    self.x0 = None
